...
// remove um comunicado publicado
export async function deleteBroadcast(broadcastId) {
    if (!broadcastId) return false;
    const docs = await listDocsRaw(COL_BROADCASTS, 500);
    const alvo = docs.find(d => d.data && d.data.id === broadcastId);
    if (!alvo) return false;
    const ok = await deleteDoc(COL_BROADCASTS, alvo._id);
    cacheClear();
    return ok;
}
...
// busca uma mensagem pelo id legivel (campo id, nao o id do documento)
export async function getMessage(messageId) {
    if (!messageId) return null;
    const docs = await listDocsRaw(COL_MESSAGES, 500);
    const achou = docs.find(d => d.data && d.data.id === messageId);
    return achou ? { _docId: achou._id, ...achou.data } : null;
}

// aplica mudancas numa mensagem (status, resposta, arquivamento)
export async function updateMessage(messageId, mudancas) {
    const atual = await getMessage(messageId);
    if (!atual) return null;
    const docIdAtual = atual._docId;
    const limpo = { ...atual };
    delete limpo._docId;
    const atualizado = { ...limpo, ...(mudancas || {}), updatedAt: new Date().toISOString() };
    const ok = await upsertDoc(COL_MESSAGES, docIdAtual, atualizado);
    cacheClear();
    return ok ? atualizado : null;
}

// remove a mensagem (exclusao definitiva)
export async function deleteMessage(messageId) {
    const atual = await getMessage(messageId);
    if (!atual) return false;
    const ok = await deleteDoc(COL_MESSAGES, atual._docId);
    cacheClear();
    return ok;
}
