...
const ACOES = ['read', 'reply', 'archive', 'unarchive', 'delete', 'hide-student', 'unhide-student'];
...
export default async function handler(req, res) {
    ...
    try {
        ...
        // some da lista do aluno, mas continua no painel da coordenacao
        if (acao === 'hide-student') mudancas = { ocultadaParaAluno: true, hiddenAt: new Date().toISOString() };
        if (acao === 'unhide-student') mudancas = { ocultadaParaAluno: false };
        if (acao === 'reply') {
        ...
        const mensagens = {
            ...
            'hide-student': 'Mensagem excluída da sua caixa.',
            'unhide-student': 'Mensagem devolvida para a sua caixa.'
        };
