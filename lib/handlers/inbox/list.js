...
export default async function handler(req, res) {
    ...
    try {
        ...
        const visiveis = minhas.filter(m => !m.ocultadaParaAluno);
        const arquivadas = visiveis.filter(m => m.status === 'arquivada');
        const ativas = visiveis.filter(m => m.status !== 'arquivada');
