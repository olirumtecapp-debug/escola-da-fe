...
// Marca o aluno como apoiador NO SERVIDOR assim que o pagamento e confirmado.
// Antes isso dependia do navegador do aluno estar aberto (polling): se ele fechasse a pagina,
// o apoio se perdia. Agora vale sempre e aparece para a coordenacao.
async function marcarApoiadorNoServidor(email) {
    if (!email || !String(email).includes('@')) return false;
    try {
        const chave = String(email).trim().toLowerCase();
        const aluno = await getStudent(chave);
        if (!aluno) return false;
        if (aluno.hasSupported) return true;
        await saveStudent({ ...aluno, email: chave, hasSupported: true, supportedAt: new Date().toISOString() });
        console.log('[Asaas] apoio registrado no servidor para ' + chave);
        return true;
    } catch (e) {
        console.error('[Asaas] falha ao marcar apoiador:', e && e.message);
        return false;
    }
}
    ...
    if (req.method === 'POST') {
        try {
            ...
            if (EVENTOS_APROVADOS.includes(event)) {
                ...
                // apoio confirmado: registra no servidor (nao depende do navegador do aluno)
                if (payment && payment.customer && payment.customer.email) {
                    marcarApoiadorNoServidor(payment.customer.email);
                }
            }
